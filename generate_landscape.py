...
def mow(grid, i, j):
  ...
  while (len(directions) > 0):
    ...
    if direction == 'U':
      if j - 2 < 0 or i + 1 >= len(grid[0]):
        continue
      elif grid[j - 2][i] == 'wall' and grid[j - 2][i + 1] == 'wall':
        grid[j - 1][i] = 'empty'
        grid[j - 2][i] = 'empty'
        grid[j - 1][i + 1] = 'empty'
        grid[j - 2][i + 1] = 'empty'
      ...
      if j + 2 >= len(grid) or i + 1 >= len(grid[0]):
        continue
      elif grid[j + 2][i] == 'wall' and grid[j + 2][i + 1] == 'wall':
        grid[j + 1][i] = 'empty'
        grid[j + 2][i] = 'empty'
      ...
      if i - 2 < 0 or j + 1 >= len(grid):
        continue
      elif grid[j][i - 2] == 'wall' and grid[j + 1][i - 2] == 'wall':
        grid[j][i - 1] = 'empty'
        grid[j][i - 2] = 'empty'
        grid[j + 1][i - 1] = 'empty'
        grid[j + 1][i - 2] = 'empty'
      ...
      if i + 2 >= len(grid[0]) or j + 1 >= len(grid):
        continue
      elif grid[j][i + 2] == 'wall' and grid[j + 1][i + 2] == 'wall':
        grid[j][i + 1] = 'empty'
        grid[j][i + 2] = 'empty'
...
def explore_maze(grid, start_i, start_j, swag):
  ...
  while queue:
    j, i = queue.pop(0)
    if grid[j][i] != 'start' and randint(1, 20) == 5:
